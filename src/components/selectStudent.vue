...
<template>
  <div class="selectClass">
    <el-select v-model="editInfo.sid"
...
<script>
import { getPersonStudentList } from '@/api/MBS/person'
...
export default {
  ...
  beforeDestroy() {
    this.terminalParams.currentPage = 1
    this.terminalParams.key = ''
  },
  // 方法集合
  methods: {
    async loadmore() {
      if (this.terminalOptions.length < this.total) {
        this.terminalParams.currentPage++
        const terminaList = await getPersonStudentList(this.terminalParams)
        this.terminalOptions = this.terminalOptions.concat(terminaList.data.qryList)
      } else {
        return
      }
    },
    ...
    async _getTerminaList() {
      ...
      const terminaList = await getPersonStudentList(this.terminalParams)
      this.terminalOptions = terminaList.data.qryList
      this.terminalOptions.forEach(item => {
        item.disabled = false
      })
      this.total = terminaList.data.totalCount
