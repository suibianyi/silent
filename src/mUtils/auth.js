...
export const BASIC_AUTHS_TREE = { // 基础权限、不可编排
  id: 999,
  children: [
    {
      id: 99901,
...
export const AUTHS_MAP = transform(concat(getLeafNodes(AUTHS_TREE), getLeafNodes(BASIC_AUTHS_TREE)), (result, value) => {
  result[value.key] = value.id
}, {})

// 是否拥有权限，authArr为数组，满足任一一个即可
export function hasAuths(authArr, userAuths) {
  if (typeof authArr === 'string') {
    authArr = [authArr]
  }
  const authCodes = authArr.map((o) => AUTHS_MAP[o])
  const commAuths = intersection(userAuths, authCodes)
  console.log('commAuths', commAuths)
