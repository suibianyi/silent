<template>
  <div class="dynamic-config">
    <van-popup v-model="showPopup" :round="true" :style="{ width: '90%', height: '80%' }" :closeable="true" @close="closePopup">
      <div class="popup-content">
        <div class="form">
          ...
          />
          <van-field
            v-model="response.descListCor"
            readonly
            name="desc"
            label="对应字段"
            @click="query('desc')"
          />
          <van-field :value="formatPageListShow(dynamicJumpPage)" label="跳转页面" placeholder="请选择跳转页面" readonly @click="clickPage()" />
          <div style="margin: 16px;">
    ...
    <tree v-if="showTree" :show="showTree" :list="treeList" @result="chooseResult"
          @on-close="closePage" />
...
export default {
  ...
  watch: {
    ...
    configIndex: {
      handler(newVal) {
        ...
        if (this.$store.getters.pageList[this.$store.getters.currentPage].componentList[newVal].dynamicJumpPage) {
          this.dynamicJumpPage = this.$store.getters.pageList[this.$store.getters.currentPage].componentList[newVal].dynamicJumpPage
          console.log('这里的dynamicJumpPage', this.dynamicJumpPage)
        }
      },
      deep: true,
      immediate: true
  ...
  methods: {
    ...
    confirm() {
      ...
      this.$store.dispatch('editComponent', { index: this.configIndex, pageListName: this.$store.getters.currentPage, key: 'dynamicJumpPage', value: this.dynamicJumpPage })
