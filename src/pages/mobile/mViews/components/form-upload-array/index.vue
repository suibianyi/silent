...
export default {
  ...
  watch: {
    value: {
      handler(newVal) {
        console.log('收到的newVal', newVal)
        if (newVal.text.length) {
          const res = []
          res.push({ url: newVal.text })
          console.log('格式化后的fileList', res)
          this.fileList = res
        } else if (newVal.value.length) {
          const res = []
          for (const item of newVal.value) {
            res.push({ url: item })
          }
          console.log('格式化后的fileList', res)
          this.fileList = res
  ...
  methods: {
    ...
    async uploadImage(stream) {
      ...
      console.log('这里是数组上传', [`${this.fileName}`])
      this.$emit('input', { value: [`${this.fileName}`], text: [`${this.uploadUrl}/${this.fileName}`] })
    },
