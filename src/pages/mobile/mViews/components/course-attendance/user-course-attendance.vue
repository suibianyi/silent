                ...
                <div class="album-content-flex">
                  ...
                  <div class="album-right">
                    <div style="color:#DA532C;font-size:32px;margin-top: 7px;">
                      ...
                      <div v-if="item.attendStatus=='absent' && item.attendanceStatus==1">
                        考勤中
                      </div>
                      <div v-if="item.attendStatus=='leave'">
                        请假
                      </div>
                      <div v-if="item.attendStatus=='late'">
                        {{ item.checkTime }}迟到
                      </div>
                      <div v-if="item.attendStatus=='normal'">
                        {{ item.checkTime }}正常
                      </div>
                    </div>
  ...
  methods: {
    ...
    async getEventStatisticsRecord() {
      const data = {
        ...
        personNo: this.search.personNo
      }
      console.log('请求的数据是', data)
      const res = await mRequest({
        url: '/mas/statistics/course/studentCourseRecord',
        method: 'post',
        data: data
      })
      ...
      console.log('this.statisticsList.length的长度是', this.statisticsList.length)
      if (this.statisticsList.length >= res.data.totalCount) {
        this.attendance.finished = true
      }
      // return getEventPerson(data)
      //   .then((res) => {
    ...
    click(item) {
      ...
      // uni.navigateTo({
      //   url: '../event-attendance-classify/event-attendance-classify?detailInfo=' +
