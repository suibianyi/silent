<template>
  <div class="dynamic-title-config">
    <van-popup v-model="showPopup" :round="true" :style="{ width: '90%', height: '80%' }" @close="closePopup">
      <div class="form">
        显示页面设置
        <van-field v-model="attendanceShow" label="进入页面" placeholder="请输入文字" readonly @click="clickPage(index)" />
  ...
  data() {
    return {
      ...
      actionSheet: {
        show: false,
        actions: [{
          value: 'event-attendance-statistics',
          name: '考勤管理-事件考勤'
        }, {
          value: 'user-event-attendance',
          name: '学生查看事件考勤'
        }]
      },
    ...
    configIndex: {
      handler(newVal) {
        if (this.$store.getters.pageList[this.$store.getters.currentPage].componentList[newVal].attendance) {
          ...
          if (this.attendance.firstPage === 'event-attendance-statistics') {
