    ...
    <van-popup v-model="showPopup" :round="true" :style="{ width: '90%', height: '80%' }" :closeable="true" @close="closePopup">
      <div class="popup-content">
        <div class="form">
          ...
          />
          <van-field
            v-model="response.textListCor"
            readonly
            name="对应字段"
            label="对应字段"
            @click="query('text')"
          />
          <van-field :value="formatPageListShow(noticeClick)" label="跳转页面" placeholder="请选择跳转页面" readonly @click="clickPage()" />
          <div style="margin: 16px;">
...
export default {
  ...
  watch: {
    ...
    configIndex: {
      handler(newVal) {
        ...
        this.noticeClick = this.$store.getters.pageList[this.$store.getters.currentPage].componentList[newVal].noticeClick
      },
      deep: true,
      immediate: true
    }
  ...
  methods: {
    ...
    chooseResult(val) {
      ...
      if (this.choose === 'text') {
        this.response.textList = str.list
        this.response.textListCor = str.cor
      }
    },
    closePopup() {
      this.$emit('on-close')
    ...
    confirm() {
      ...
      this.$store.dispatch('editComponent', { index: this.configIndex, pageListName: this.$store.getters.currentPage, key: 'noticeClick', value: this.noticeClick })
