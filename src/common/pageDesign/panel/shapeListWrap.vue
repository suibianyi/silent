<template>
  <div id="shape-list-wrap">
    <img-water-fall class="shape-list" :list-data="shapeList" sort-by="id" k="id" @select-img="selectShape" />
  </div>
</template>
...
export default {
  ...
  watch: {},
  created() {
    const line = JSON.parse(JSON.stringify(wLine.setting))
    this.list[0].setting = line
    const line2 = JSON.parse(JSON.stringify(wLine.setting))
    line2.width = 2
    line2.height = 300
    this.list[1].setting = line2
    const circle = JSON.parse(JSON.stringify(wCircle.setting))
    this.list[2].setting = circle
    const circle2 = JSON.parse(JSON.stringify(wCircle.setting))
    circle2.backgroundColor = 'rgba(0, 0, 0, 1)'
    this.list[3].setting = circle2
    const rectangle = JSON.parse(JSON.stringify(wRectangle.setting))
    this.list[4].setting = rectangle
    const rectangle2 = JSON.parse(JSON.stringify(wRectangle.setting))
